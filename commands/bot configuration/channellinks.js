...
module.exports = {
    ...
    async execute(interaction) {
        ...
        channelLinksEmbed.addFields({name: 'Sealog Channel:', value: sealogChannel ? `<#${sealogChannel.id}>` : "not linked"})
        channelLinksEmbed.addFields({name: 'PromoLog Channel:', value: `${promologChannel ? `<#${promologChannel.id}>` : "not linked"}`})
        channelLinksEmbed.addFields({name: 'ExpDisplay Channel:', value: `${expdisplayChannel ? `<#${expdisplayChannel.id}>` : "not linked"}`})
        channelLinksEmbed.addFields({name: 'Training Channels:', value: `${trainingChannels.length > 0 ? trainingChannels.map(channel => `<#${channel.id}>`).join("\n") : "none linked"}`})
        channelLinksEmbed.addFields({name: 'Patrol Channels:', value: `${patrolChannels.length > 0 ? patrolChannels.map(channel => `<#${channel.id}>`).join("\n") : "none linked"}`})
        channelLinksEmbed.addFields({name: 'Gamenight Channels:', value: `${gamenightChannels.length > 0 ? gamenightChannels.map(channel => `<#${channel.id}>`).join("\n") : "none linked"}`})
